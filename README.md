...
# Binary
NN and CNN were trained on them for the reason I made for a hypothesis that NN and CNN would perform better againt the drawing interface, if their data of training have the same type of data as the drawing grid vector.
Binary data are like boolean data. If the pixel have a number higher than 0, it's new value is 1.

# Grayscale
Grayscale is a float. The number the pixel can get is between 0 and 1, depending how dark the pixel is. It's more rich in information than the binary one.
...
PyTorch accelerate the training. In this model, the image go trough the filters, reLU, and then the pool (Maxpool). I trained two networks with two type of the same data, one binary and the other one grayscale. The binary one have an accuracy of 98.21% with binary test data and 97.62% with grayscale test data. For the grayscale one, I got an accuracy of 96.57% for binary data and 98.61% for grayscale data. For the drawing interface, you would assume SCNN1 (the grayscale one) would be better, but it's the opposite. SCNN2 outperformed SCNN1. SCNN1 got 60% accuracy and SCNN2 got 64% accuracy.
